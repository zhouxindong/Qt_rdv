...
namespace ssa
{
	...
	*******************************************************************************/
	...
	public:
		...
		*/
		...
		@par 示例
			...
			@code
				...
				std::map<xmString, xmPtr<xmValue>> mapValue;
				v1.Unstructured(mapValue);
				std::map<xmString, xmPtr<xmValue>>::iterator pos;
				for (pos = mapValue.begin(); pos != mapValue.end(); pos++)
				{
					printf("%s = %s\n", pos->first.String(), pos->second->PrintValue().String());
				}
			...
			@endcode
		*/
		void Unstructured(std::map<xmString, xmPtr<xmValue>>& mapValue)
		{
			mapValue.clear();
			if (Type() != xmDT_STRUCT)
			{
			...
			for (size_t i = 0; i < PropertyCount(); i++)
			{
				...
				{
					std::map<xmString, xmPtr<xmValue>> mapProptyValue;
					pValue->Unstructured(mapProptyValue);

					xmString strProptyName = xmStringUtil::Format("%s.", PropertyName(i));				
					std::map<xmString, xmPtr<xmValue>>::iterator pos;
					for (pos = mapProptyValue.begin(); pos != mapProptyValue.end(); pos++)
					{
						mapValue.insert(std::make_pair(strProptyName+pos->first, pos->second));
					}
				}
				else
				{
					mapValue.insert(std::make_pair(PropertyName(i), pValue));
				}
			}
		}
		...
		*/
//		const char& i8() const { xmAssert(xmDT_INT8 == Type()); return __V<char>(); }
//		char& i8() { return const_cast<char&>(static_cast<const xmValue&>(*this).i8()); }
//
//		/** unsigned char类型访问函数
//		@return 作为unsigned char类型的引用。
//		@see const char& i8() const
//		*/
//		const unsigned char& u8() const { xmAssert(xmDT_UINT8 == Type()); return __V<unsigned char>(); }
//		unsigned char& u8() { return const_cast<unsigned char&>(static_cast<const xmValue&>(*this).u8()); }
//
//		/** short类型访问函数
//		@return 作为short类型的引用。
//		@see const char& i8() const
//		*/
//		const short& i16() const { xmAssert(xmDT_INT16 == Type()); return __V<short>(); }
//		short& i16() { return const_cast<short&>(static_cast<const xmValue&>(*this).i16()); }
//
//		/** unsigned short类型访问函数
//		@return 作为unsigned short类型的引用。
//		@see const char& i8() const
//		*/
//		const unsigned short& u16() const { xmAssert(xmDT_UINT16 == Type()); return __V<unsigned short>(); }
//		unsigned short& u16() { return const_cast<unsigned short&>(static_cast<const xmValue&>(*this).u16()); }
//
//		/** int类型访问函数
//		@return 作为int类型的引用。
//		@see const char& i8() const
//		*/
//		const int& i32() const { xmAssert(xmDT_INT32 == Type()); return __V<int>(); }
//		int& i32() { return const_cast<int&>(static_cast<const xmValue&>(*this).i32()); }
//
//		/** unsigned int类型访问函数
//		@return 作为unsigned int类型的引用。
//		@see const char& i8() const
//		*/
//		const unsigned int& u32() const { xmAssert(xmDT_UINT32 == Type()); return __V<unsigned int>(); }
//		unsigned int& u32() { return const_cast<unsigned int&>(static_cast<const xmValue&>(*this).u32()); }
//
//		/**@fn i64() 
//		64位长整型访问函数
//		@return 作为64位长整型的引用。
...
//		/**@fn u64()
//		64位无符号长整型访问函数
//		@return 作为64位无符号长整型的引用。
//		@see const char& i8() const
//		*/
//#ifdef xmPLATFORM_X64
//		const long& i64() const { xmAssert(xmDT_INT64 == Type()); return __V<long>(); }
//		long& i64() { return const_cast<long&>(static_cast<const xmValue&>(*this).i64()); }
//
//		const unsigned long& u64() const { xmAssert(xmDT_UINT64 == Type()); return __V<unsigned long>(); }
//		unsigned long& u64() { return const_cast<unsigned long&>(static_cast<const xmValue&>(*this).u64()); }
//#else
//		const long long& i64() const { xmAssert(xmDT_INT64 == Type()); return __V<long long>(); }
//		long long& i64() { return const_cast<long long&>(static_cast<const xmValue&>(*this).i64()); }
//
//		const unsigned long long& u64() const { xmAssert(xmDT_UINT64 == Type()); return __V<unsigned long long>(); }
//		unsigned long long& u64() { return const_cast<unsigned long long&>(static_cast<const xmValue&>(*this).u64()); }
//#endif
//
//		/** float类型访问函数
//		@return 作为float类型的引用。
//		@see const char& i8() const
//		*/
//		const float& f32() const { xmAssert(xmDT_FLOAT == Type()); return __V<float>(); }
//		float& f32() { return const_cast<float&>(static_cast<const xmValue&>(*this).f32()); }
//
//		/** double类型访问函数
//		@return 作为double类型的引用。
//		@see const char& i8() const
//		*/
//		const double& f64() const { xmAssert(xmDT_DOUBLE == Type()); return __V<double>(); }
//		double& f64() { return const_cast<double&>(static_cast<const xmValue&>(*this).f64()); }
		
//#define __DEFINE_GeterSeter(T, Func) \
//	inline const T##& Func##() const {xmAssertAlways(xmIsTypeT<T>::TYPE == Type()); return __V<T>();}	\
...
#undef __DEFINE_GeterSeter
		...
		*/
		...
		@exception xmExIllegalInput
			当uPos超出属性数量范围时抛出。
		*/
		inline const char* PropertyName(size_t uPos) const
		{
		...
		*/
		inline xmValue PropertyValue(size_t uPos) const
		{
		...
		*/
		...
		inline xmValue PropertyValue(const xmStringVector& vPath, size_t uStartPos = 0) const
		{
			if (vPath.Size() == 0)
			{
				return xmValue(m_pBuffer, *m_pClass, false);
			...
			const xmClass* pClass = m_pClass->PropertyClass(vPath, uStartPos);
			if (pClass == NULL)
			{
				return xmValue();
			}
