...
namespace ssa
{
	...
	*******************************************************************************/
	...
	public:
		...
		*/
		...
		@param [in] uBufferSize 
			当eType为xmDT_BUFFER时，由uBufferSize指定Buffer的长度，否则忽略该参数。
		@exception xmExIllegalInput
			当eType为xmDT_NULL、xmDT_STRUCT，或者eType为xmDT_BUFFER但uBufferSize为0时抛出。
		@par 示例
			@par
			@code
	...
	public:
		...
		*/
		inline const xmClass* PropertyClass(size_t uPos) const;
		...
		*/
		...
		const xmClass* PropertyClass(const xmStringVector& vPath, size_t uStartPos = 0) const;
		...
		*/
		inline const xmClass& operator[](size_t uPos) const
		{
