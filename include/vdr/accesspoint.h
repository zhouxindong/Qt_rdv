...
namespace ssa
{
	...
	*/
	...
	public:
		...
		* @param strNode 是指被代表的节点的名字。
		* @return 授权结果，true表示成功，false表示失败。
		*/
		virtual bool           Authorize(const char* strNode);
		/**
		* 数据访问接口取消节点在VDR的操作权限授权。
		* @param strNode 是指被代表的节点的名字。
		* @return 取消授权结果，true表示成功，false表示失败。
		*/
		virtual bool           UnAuthorize(const char* strNode);
		/**
		* 获取数据访问接口授权的节点。
		...
		* @param strDataSet 是指被依附的数据集的名字。
		* @param nTry 是指被尝试多少次。
		* @return 依附结果，true表示成功，false表示失败。
		*/
		virtual bool           Attach(xmVDR* pVdr, const char* strDataSet = "", int nTry = 65535);
		/**
		* 与节点授权类似，获取授权最终是获取对数据集的操作。此接口用于将数据访问接口依附在数据集上。
		* 也可以通过Node的Authorize进行授权，推荐用法是通过Node进行授权。
		* @param strDataSet 是指被依附的数据集的名字。
		...
		* @return 依附结果，true表示成功，false表示失败。
		*/
		virtual bool           Attach(const char* strDataSet, int nTry = 65535);
		/**
		* 取消数据访问接口对数据集的依附。
		* @param strDataSet 是指被依附的数据集的名字。
		* @return 取消依附结果，true表示成功，false表示失败。
		*/
		virtual bool           UnAttach(const char* strDataSet);
		/**
		* 获取数据访问接口依附的数据集合信息。
		...
		*/
		bool                   IsValid();
		/**
		* 对于本接入点，使某个数据集具备或者不具备发布数据值的能力。
		* @param strDataSet 是指数据集的名字。
		* @param havePublishAbility 是指数据集是否具有发布数据值的能力。
		* @return 无。
		*/
		void                   SetPublishAbility(const std::string& strDataSet, bool havePublishAbility);
		/**
		* 如果系统中存在多个节点同时写一个数据或者数据集的情况，首先要获取权限，解决多个写之间的冲突问题。
		* @param strDataSet 是指同时写的数据集的名字。
		* @return 结果，true表示成功，false表示失败。
		*/
		virtual bool           GetToken(const char* strDataSet = "");
		/**
		...
		* @return 结果，0表示成功，-1表示失败，其他代码表示需要用户根据逻辑判断是否正确。
		*/
		virtual int            ConfigDataEventHandler(const char* strName, xmEDataEvent deDataEvent, xmMsgHandler* pHandler = 0);
		/**
		...
		* @return 结果，0表示成功，-1表示失败，其他代码表示需要用户根据逻辑判断是否正确。
		*/
		virtual int            GetValue(const char* strName, xmValue& bValue, const char* flag = ".");
		/**
		* 使用.号分割名字,char*形式获取数值，使用时可以只使用最顶层的数据名字。
		* @param strName 数据名字。
		* @param bValue 以char*形式存储参数值的缓冲区地址。
		* @param uLength 获取数据值的大小，例如byte为1个字节，int为4个字节，float为4个字节，以此类推。
		* @param flag  在结构化数据中名字路径分割符号，默认均为“.”。
		* @return 结果，0表示成功，-1表示失败，其他代码表示需要用户根据逻辑判断是否正确。
		*/
		virtual int            GetValue(const char* strName, char* bValue, unsigned int uLength, const char* flag = ".");
		/**
		...
		*/
		template<class T>
		int GetValue(const char* strName, T& Value, const char* flag = ".")
		{
			return GetValue(strName, (char*)&Value, sizeof(T), flag);
		}
		/**
		* 判断数据是否有更新。
		* @param strName 数据名字。
		...
		* @return 结果，0表示成功，-1表示失败，其他代码表示需要用户根据逻辑判断是否正确。
		*/
		virtual int            SetValue(const char* strName, const xmValue& bValue, const char* flag = ".");
		/**
		* 使用.号分割名字,char*形式写数值，使用时可以只使用最顶层的数据名字。
		* @param strName 数据名字。
		* @param bValue 以char*形式存储参数值的缓冲区地址。
		* @param uLength 写数据值的大小，例如byte为1个字节，int为4个字节，float为4个字节，以此类推。
		* @param flag  在结构化数据中名字路径分割符号，默认均为“.”。
		* @return 结果，0表示成功，-1表示失败，其他代码表示需要用户根据逻辑判断是否正确。
		*/
		virtual int            SetValue(const char* strName, const char* bValue, unsigned int uLength, const char* flag = ".");
		/**
		...
		*/
		template<class T>
		int SetValue(const char* strName, const T& Value, const char* flag = ".")
		{
			return SetValue(strName, (char*)&Value, sizeof(T), flag);
		}
