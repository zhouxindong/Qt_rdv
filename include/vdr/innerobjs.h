...
namespace ssa
{
	...
	public:
		...
		// 所属数据集合的指针
		xmDataSetWithDetail*     m_pDataSet;
		//按照数据集合中数据排序后获取的位置，按照字节排序。
		unsigned int             m_uIndexOffset;
		//目前只是预留，原打算用于更新状态时计数
		int                      m_nCounter;
		//对应的内存空间的指针，为了快速检索
		xmMemoryIndex            m_MemoryIndex;
		xmMemoryPage*            m_pMainMemoryPage;
		//本地内存，用于临时存储本身所需要发布的数据值
		xmMemoryPage*            m_pMirrorMemoryPage;
		//xmMemoryIndex  m_MirrorMemoryTableIndex;
		//分配内存的初始地址
		//留作Value类型的指针
		void*                    m_lpValueMainAddr;
		//留作Value类型的指针
		void*                    m_lpValueMirrorAddr;
	private:
		//xmMutex                  m_Mutex;
	...
	public:
		...
		//是否具有发布数据值的权限
		bool                                 m_bHavePublishValueAbility;
	...
	public:
		...
		//复位数值，用于几个周期后，自动复位数据。
		void                  DataReset2InitState(bool bMirrorMemory = false);
		void                  SyncMemorys();
