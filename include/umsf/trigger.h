...
namespace ssa
{
	...
	class xmClickTriggerContainer : public xmMsgHandler, public xmTriggerContainer
	{
	public:
		xmClickTriggerContainer() : xmTriggerContainer() {};
		virtual ~xmClickTriggerContainer()
		{
			xmMH_DESTRUCT_IMMEDIATELY();	// or xmMH_DESTRUCT_WAIT_IDLE();
		};
		virtual xmEMsgHandleObject Type() const
		{
			return MHO_CLICK_TRIGGER;
		}
	...
	public:
		...
		unsigned int GetTriggerCycle(size_t uPos) const;
	...
	private:
		...
		{
			xmLog::info("ClickTrigger", "Work load %.2f%% pre second.", ((xmMsgHandler*)pParam)->WorkLoad());
		}
	private:
		void __OnWorldClick(const smWorldClick& aMsg, std::multimap<unsigned int, xmTrigger*>::iterator& pos);
		void OnWorldClick(const smWorldClick& aMsg);
		xmCREATE_MESSAGE_MAP(xmClickTriggerContainer);
	};
