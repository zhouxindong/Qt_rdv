...
/** @file umsf.h
@brief
	定义xmUMSF（Unified Model Schedule Framework）类。
...
namespace ssa
{
	...
	public:
		...
		//	失败抛出异常
		void Boot(const char* strUmsfConfigFile, const char* strModelConfigFile);
		void Halt();
		void Reboot(const char* strUmsfConfigFile = NULL, const char* strModelConfigFile = NULL);
		...
		xmString ClickTriggerPath(size_t uTriggerID);
		...
		//	对消息队列的长度和负载统计
		//	Manager、ClickTrigger、EventTrigger
